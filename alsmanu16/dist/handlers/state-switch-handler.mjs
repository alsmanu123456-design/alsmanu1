...
export async function handleText(bot2, msg) {
  ...
  switch (state.state) {
    ...
    // [SEC-REBUILD] القفل أصبح حقيقياً — نفتح الجلسة فوراً بعد الإعداد
    case "awaiting_security_pin": {
      ...
      saveUser(userId, { securitySettings: { ...sec, pin: text }, secUnlockedAt: Date.now() });
      clearState(userId);
      await bot2.sendMessage(chatId,
        "✅ *تم إعداد قفل PIN بنجاح!*\n\n🔐 من الآن لن يُفتح قسم الأمان إلا بهذا الرمز.",
        { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🛡️ فتح قسم الأمان", callback_data: "menu_security" }]] } });
      return true;
    }
    ...
    case "awaiting_security_pin_change": {
      ...
      saveUser(userId, { securitySettings: { ...secChg, pin: text }, secUnlockedAt: Date.now() });
      clearState(userId);
      await bot2.sendMessage(
        chatId,
        "🔄 *تم تغيير PIN بنجاح!*\n\n🔐 قسم الأمان محمي بالرمز الجديد.",
        { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🛡️ فتح قسم الأمان", callback_data: "menu_security" }]] } }
      );
      return true;
    }
    // ── Security: unlock PIN gate ─────────────────────────────────────────────
    // [SEC-REBUILD] بوابة الدخول لقسم الأمان — صلاحية الفتح 10 دقائق
    case "awaiting_security_pin_unlock": {
      const secUnl = user.securitySettings || {};
      if (text !== secUnl.pin) {
        await bot2.sendMessage(chatId, "❌ رمز خاطئ — حاول مرة أخرى:", { reply_markup: cancelKeyboard() });
        return true;
      }
      clearState(userId);
      saveUser(userId, { secUnlockedAt: Date.now() });
      if (_deps.handleSecurityCallback) {
        await _deps.handleSecurityCallback(bot2, chatId, userId, "menu_security");
      } else {
        await bot2.sendMessage(chatId, "🔓 تم فتح القفل", { reply_markup: { inline_keyboard: [[{ text: "🛡️ قسم الأمان", callback_data: "menu_security" }]] } });
      }
      return true;
    }
    // ── Security: block number for real on WhatsApp ───────────────────────────
    // [SEC-REBUILD] حظر حقيقي عبر updateBlockStatus بدلاً من علم محلي بلا أثر
    case "awaiting_security_block_num": {
      const numBlk = text.replace(/\D/g, "");
      clearState(userId);
      if (!numBlk || numBlk.length < 7) {
        await bot2.sendMessage(chatId,
          "❌ *رقم غير صحيح*\n\nأدخل الرقم مع كود الدولة بدون +\nمثال: `249912345678`",
          { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "menu_security" }]] } });
        return true;
      }
      const sockBlk = inMemoryDB.sessions.get(userId);
      if (!sockBlk) {
        await bot2.sendMessage(chatId, "🔴 واتساب غير متصل — اربط رقمك أولاً.", { reply_markup: { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "menu_security" }]] } });
        return true;
      }
      try {
        await sockBlk.updateBlockStatus(`${numBlk}@s.whatsapp.net`, "block");
        await bot2.sendMessage(chatId,
          `🚫 *تم الحظر بنجاح!*\n\n+${numBlk} محظور الآن على واتساب — لن يستطيع مراسلتك أو الاتصال بك.`,
          { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "📵 قائمة الحظر", callback_data: "sec_blocklist" }], [{ text: "🔙 رجوع", callback_data: "menu_security" }]] } });
      } catch (errBlk) {
        await bot2.sendMessage(chatId, `❌ تعذّر الحظر: ${errBlk?.message || "خطأ غير معروف"}`);
      }
      return true;
    }
